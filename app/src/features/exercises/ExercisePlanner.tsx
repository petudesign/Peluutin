...
export function ExercisePlanner({ team, theme, shared = false, onBack, onToggleTheme }: ExercisePlannerProps) {
  ...
  const exportExerciseImage = async () => {
    try {
      ...
      setPlaybackElapsedMs(0);
      setShareMenuOpen(false);
      const pitchCanvas = document.querySelector<HTMLCanvasElement>(".exercise-stage .exercise-canvas");
      ...
      window.setTimeout(() => setImageExportLabel("Lataa kuvana (PNG)"), 1800);
    } catch {
